...
<x-guest-layout>
        ...
        <div class="w-full p-6 flex">
            <div style="margin-left: auto; width: 40%;">
                ...
                <div class="mt-4">
                    ...
                    <input class="block w-full shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm border-gray-300 rounded-md @error('lastname') is-invalid @enderror" id="lastname" value="{{ old('lastname') ? old('lastname') : $user->lastname }}" name="lastname" type="text" placeholder="{{ __('admin.words.lastname') }}" aria-label="{{ __('admin.words.lastname') }}">
                </div>
                <div class="mt-4">
                    <label class="block text-sm text-gray-600" for="firstname">{{ __('admin.words.firstname') }}</label>
                    <input class="block w-full shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm border-gray-300 rounded-md @error('firstname') is-invalid @enderror" id="firstname" value="{{ old('firstname') ? old('firstname') : $user->firstname }}" name="firstname" type="text" placeholder="{{ __('admin.words.firstname') }}" aria-label="{{ __('admin.words.firstname') }}">
                </div>
                <div class="mt-4">
                    <label class="block text-sm text-gray-600" for="email">{{ __('admin.words.email') }}</label>
                    <input class="block w-full shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm border-gray-300 rounded-md @error('email') is-invalid @enderror" id="email" value="{{ old('email') ? old('email') : $user->email }}" name="email" type="email" inputmode="email" placeholder="{{ __('admin.words.email') }}" aria-label="E-mail">
                </div>
                <div class="mt-4">
                    <label class=" block text-sm text-gray-600" for="address">{{ __('admin.users.address') }}</label>
                    <input class="block w-full shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm border-gray-300 rounded-md @error('address') is-invalid @enderror" id="address" value="{{ old('address') ? old('address') : $user->address }}" name="address" type="text" placeholder="{{ __('admin.users.address') }}" aria-label="{{ __('admin.users.address') }}">
                </div>
                <div class="mt-2">
                    <label class="hidden text-sm block text-gray-600" for="city">{{ __('admin.users.city') }}</label>
                    <input class="block w-full shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm border-gray-300 rounded-md @error('city') is-invalid @enderror" id="city" value="{{ old('city') ? old('city') : $user->city }}" name="city" type="text" placeholder="{{ __('admin.users.city') }}" aria-label="{{ __('admin.users.city') }}">
                </div>
                <div class="flex justify-between mt-2">
                    <div class="inline-block -mx-1 pl-1 w-1/2">
                        ...
                        <input class="block w-full shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm border-gray-300 rounded-md @error('zipcode') is-invalid @enderror" id="zipcode" value="{{ old('zipcode') ? old('zipcode') : $user->zipcode }}"  name="zipcode" type="number" inputmode="numeric" placeholder="{{ __('admin.users.zipcode') }}" aria-label="{{ __('admin.users.zipcode') }}">
                    </div>
                    <div class="inline-block w-1/2">
                        <label class="hidden block text-sm text-gray-600" for="billing_country">{{ __('admin.users.country') }}</label>
                        <input class="block w-full shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm border-gray-300 rounded-md @error('billing_country') is-invalid @enderror" id="billing_country" value="{{ old('billing_country') ? old('billing_country') : $user->billing_country }}" name="billing_country" type="text" placeholder="{{ __('admin.users.country') }}" aria-label="{{ __('admin.users.country') }}">
                    </div>
                </div>
                    ...
                    <fieldset class="mt-4">
                        ...
                        <div class="space-y-4 sm:flex sm:items-center sm:space-y-0 sm:space-x-10">
                            <div class="flex items-center">
                                <input id="coinbase" name="coinbase" type="radio" wire:model="coinbase" value="coinbase" class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300">
                            ...
                            <div class="flex items-center">
                                <input id="stripe" name="coinbase" type="radio" checked wire:model="coinbase" value="stripe" class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300">
    ...
    @if(isset($coinbase) && !empty($coinbase) && $coinbase === 'stripe')
        @section('javascript')
