        ...
        <div>
            @foreach($biens as $key => $bien)
                <a href="{{ route('biens', ['slug' => $bien->slug]) }}" class="mt-10 group flex flex-col justify-between bg-white relative overflow-hidden shadow-xl rounded-lg {{ $key % 2 === 1 ? 'lg:flex-row-reverse' : 'lg:flex-row' }}">
                    ...
                    @endif
                    <div class="@if($bien->total_tokens === 0) opacity-25 @endif lg:w-1/2 max-h-96 w-full overflow-hidden">
                        <img src="{{ URL::asset(/*'storage/' . */$bien->mainImage()->image) }}" alt="Tall slender porcelain bottle with natural clay textured body and cork stopper." class="w-full h-full object-center object-cover group-hover:opacity-75">
                    </div>
