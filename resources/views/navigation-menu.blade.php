            ...
            <div class="flex items-center">
                ...
                <div class="hidden ml-10 space-x-8 lg:flex">
                    ...
                    <div x-data="{dropdownMenu: false}" class="relative">
                        ...
                        <!-- Dropdown list -->
                        <div x-show="dropdownMenu" class="absolute py-2 mt-2 bg-white bg-gray-100 rounded-md shadow-xl z-50" style="transform: translateX(-50%); left: 50%;">
                    ...
                    <div class="flex items-center justify-between text-center gap-2">
                        @auth
                            <a href="{{ route(\Illuminate\Support\Facades\Auth::user()->is_admin === 1 ? 'admin.dashboard' : 'dashboard') }}" class="text-base font-medium text-white hover:text-indigo-50">
                                <img class="rounded-full" width="30px" height="30px" style="width: 30px; height: 30px;" src="{{ \Illuminate\Support\Facades\Auth::user()->profile_photo_path ? \Illuminate\Support\Facades\Storage::url(\Illuminate\Support\Facades\Auth::user()->profile_photo_path) : 'https://cdn-icons-png.flaticon.com/512/149/149071.png' }}" alt="Profile">
                            </a>
                        @else
                            <a href="{{ route('login') }}" class="inline-block bg-palette-2-grey p-2 border border-transparent rounded-md text-base font-medium bg-palette-2-dark-beige text-white hover:text-palette-2-dark-blue hover:bg-indigo-50 transition-all duration-150 ease-in-out">
                                {{ __('home.pagination.my-account') }}
                            </a>
                            @if (Route::has('register'))
                                <a href="{{ route('register') }}" class="inline-block bg-palette-2-dark-blue p-2 border border-transparent rounded-md text-base font-medium bg-palette-2-dark-blue text-palette-2-light-beige hover:bg-indigo-700 transition-all duration-150 ease-in-out">
